...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  Future<void> _handleLogin() async {
    if (_emailCtrl.text.trim().isEmpty || _passCtrl.text.trim().isEmpty) { _showError('Please fill in all fields'); return; }
    ...
    try {
      ...
      if (user['role'] != 'coach' && user['role'] != 'head_coach') { _showError('Only coaches can access this page.'); return; }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 20),
            const Text('Coach Portal', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
            const SizedBox(height: 6),
            const Text('Access your coaching dashboard', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
            ...
            AppCard(
              child: Column(
                children: [
                  ...
                  TextButton(onPressed: () => context.push('/forgot-password'), child: const Text('Forgot password?', style: TextStyle(color: AppColors.textSecondary, fontSize: 13))),
                ],
              ),
            ),
          ],
